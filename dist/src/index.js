...
exports.default = {
    ...
    async bootstrap({ strapi }) {
        ...
        for (const uid of contentTypes) {
            ...
            if (!existing.has(findAction))
                toCreate.push({ action: findAction, role: publicRole.id });
            if (!existing.has(findOneAction))
                toCreate.push({ action: findOneAction, role: publicRole.id });
        }
        if (toCreate.length) {
            await Promise.all(toCreate.map((p) => strapi.query('plugin::users-permissions.permission').create({ data: p })));
